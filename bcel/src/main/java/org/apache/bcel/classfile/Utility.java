...
public abstract class Utility {
     ...
     * signatureToString(). Read by methodSignatureToString(). Set by side
     * effect,but only internally.
     */
	...
	switch (opcode) {
		...
		buf.append("\t").append(constant_pool.constantToString(c)).append((verbose ? " (" + index + ")" : ""));
		break;
	    case Constants.INVOKEINTERFACE:
		index = bytes.readUnsignedShort();
		int nargs = bytes.readUnsignedByte(); // historical, redundant
		buf.append("\t").append(constant_pool.constantToString(index, Constants.CONSTANT_InterfaceMethodref)).append(verbose ? " (" + index + ")\t" : "").append(nargs).append("\t").append(bytes.readUnsignedByte()); // Last
		// byte
		// is
		// a
		// reserved
		...
		buf.append("\t").append("<dyn>.").append(constant_pool.constantToString(id.getNameAndTypeIndex(), Constants.CONSTANT_NameAndType));
		if (verbose) {
	...
	try {
		...
		case 'L': { // Full class name
		    ...
		    // corrected concurrent private static field acess
		    wrap(consumed_chars, index + 1); // "Lblabla;" `L' and `;'
		    // are removed
		...
		case '[': { // Array declaration
		    ...
		    // Count opening brackets and look for optional size
		    // argument
    ...
    /**
     * Converts a list of AnnotationGen objects into a set of attributes that
     * can be attached to the class file.
     *
     * @param cp
     *            The constant pool gen where we can create the necessary name
     *            refs
     * @param vec
     ...
     */
    public static Attribute[] getParameterAnnotationAttributes(ConstantPoolGen cp, List<AnnotationEntryGen>[] /*
     * Array
     * of
     * lists
     * ,
    ...
    public static String getListOfInterfaces(JavaClass[] allInterfaces) {
	StringBuffer buf = new StringBuffer();
	for (JavaClass jc : allInterfaces) {
    ...
    public static String getFields(String indent, Field[] fields) {
	StringBuffer buf = new StringBuffer();
	for (Field field : fields) {
	    buf.append(indent + field + "\n");
	}
	String string = buf.toString();
	return string;
    }

    public static String getMethods(String indent, Method[] methods, String className) {
	StringBuffer buf = new StringBuffer();
	for (Method method : methods) {
	    buf.append(indent + method.toString(indent) + "{\n");
	    buf.append(indent + "/*\n");
	    buf.append(indent + method.getCode().toString(indent) + "\n");
	    buf.append(indent + "*/\n");
	    buf.append(indent + "}\n\n");
	}
	String string = buf.toString().replace("<init>", className.substring(className.lastIndexOf(".") + 1, className.length()));
	return string.substring(0, string.length() - 1);
    }
}
