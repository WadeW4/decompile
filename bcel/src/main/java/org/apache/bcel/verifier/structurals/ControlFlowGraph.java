...
public class ControlFlowGraph {
    ...
    /**
     * Objects of this class represent a node in a ControlFlowGraph. These nodes
     * are instructions, not basic blocks.
     */
    private class InstructionContextImpl implements InstructionContext {

	/**
	 * The TAG field is here for external temporary flagging, such as graph
	 * colouring.
	 *
	 * @see #getTag()
	 * @see #setTag(int)
	 ...
	 */
	private Map<InstructionContext, Object> inFrames; // key: the
	// last-executed JSR

	/**
	 * The 'outgoing' execution Frames.
	 */
	private Map<InstructionContext, Object> outFrames; // key: the
	// last-executed JSR

	/**
	 * The 'execution predecessors' - a list of type InstructionContext of
	 * those instances that have been execute()d before in that order.
	 */
	private ArrayList<InstructionContextImpl> executionPredecessors = null; // Type:
	...
	public InstructionContextImpl(InstructionHandle inst) {
	    ...
	    instruction = inst;
	    inFrames = new HashMap<InstructionContext, Object>();
	    outFrames = new HashMap<InstructionContext, Object>();
	}
	...
	/**
	 * Returns a clone of the "outgoing" frame situation with respect to the
	 * given ExecutionChain.
	 */
	@Override
	 ...
	 * executes the instructions symbolically and therefore calculates the
	 * "outgoing" frame situation. Returns: True iff the "incoming" frame
	 * situation changed after merging with "inFrame". The execPreds
	 * ArrayList must contain the InstructionContext objects executed so far
	 * in the correct order. This is just one execution path [out of many].
	 * This is needed to correctly "merge" in the special case of a RET's
	 * successor. <B>The InstConstraintVisitor and ExecutionVisitor
	 * instances must be set up correctly.</B>
	 *
	 * @return true - if and only if the "outgoing" frame situation changed
	 *         from the one before execute()ing.
	 */
	@Override
	...
	/**
	 * Does the actual merging (vmspec2, page 146). Returns true IFF
	 * this.inFrame was changed in course of merging with inFrame.
	 */
	...
	/**
	 * Returns the control flow execution chain. This is built while
	 * execute(Frame, ArrayList)-ing the code represented by the surrounding
	 * ControlFlowGraph.
	 */
	 ...
	 * at-the-end-extended message. This extended message will then reflect
	 * the execution flow needed to get to the constraint violation that
	 * triggered the throwing of the "e" object.
	 */
	...
	/**
	 * Returns the InstructionContextImpl with an JSR/JSR_W that was last in
	 * the ExecutionChain, without a corresponding RET, i.e. we were called
	 * by this one. Returns null if we were called from the top level.
	 */
	private InstructionContextImpl lastExecutionJSR() {
		...
		InstructionContextImpl current = (executionPredecessors.get(i));
	 ...
	 * InstructionHandle That means, a RET does have successors as defined
	 * here. A JsrInstruction has its target as its successor (opposed to
	 * its physical successor) as defined here.
	 */
	...
	private InstructionHandle[] _getSuccessors() {
		...
		if (s == null) { // return empty; // RET in dead code. "empty"
		    // would be the correct answer, but we know
		    // something about the surrounding project...
		    throw new AssertionViolatedException("Asking for successors of a RET in dead code?!");
		}

		// TODO: remove. Only JustIce must not use it, but foreign users
		// of the ControlFlowGraph
		// will want it. Thanks Johannes Wust.
		// throw new
		// AssertionViolatedException("DID YOU REALLY WANT TO ASK FOR RET'S SUCCS?");
    ...
    /**
     * The Subroutines object for the method whose control flow is represented
     * by this ControlFlowGraph.
     */
    ...
    /**
     * Returns true, if and only if the said instruction is not reachable; that
     * means, if it is not part of this ControlFlowGraph.
     */
